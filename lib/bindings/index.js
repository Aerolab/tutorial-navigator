...
function tutorialFetch(el, property) {
  var binding = this;
  var baseUrl = this.model.docsDomain() || base;
  var clientId = this.model.clientId() || '';
  var view = binding.view;
  var model = binding.model;

  binding.change(function(name) {
    if (!name) return;
    var platform = view.getPlatform(name);
    var api = view.getApi(name);
    var uri = platform ? platform.url : api.url;
    var docUrl = baseUrl + uri + '?e=1' + (clientId ? '&a=' + clientId : '');
